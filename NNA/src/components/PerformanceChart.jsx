...
function PerformanceChart({ graphId }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        console.log("Frontend is fetching Iteration Metrics");

        const response = await fetch(
          `http://localhost:3000/api/IterationMetric?graphId=${graphId}`
        );
        const result = await response.json();

        // Transform backend data to the format required by the chart
        const transformedData = result.map((item) => ({
          iterationNumber: item.Step,
        ...
        console.log(
          `Data from iterationmetric with GraphId ${graphId}:`,
          transformedData
        );
      } catch (error) {
